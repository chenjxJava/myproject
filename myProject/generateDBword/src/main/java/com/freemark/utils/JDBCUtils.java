...
import com.freemark.domain.Coloums;
import com.freemark.domain.Table;

//jdbc链接数据库,获取表名，字段名和数据
public class JDBCUtils {
	private static String driver = "com.mysql.jdbc.Driver";
	private static String url = "jdbc:mysql://127.0.0.1:3306/shiro?useUnicode=true&characterEncoding=UTF-8&createDatabaseIfNotExist=true";
	...
	public static List<Table> getTablesBy(Connection conn) throws SQLException {
		DatabaseMetaData dbMetData = conn.getMetaData();
		// mysql convertDatabaseCharsetType null
		ResultSet rs = dbMetData.getTables(null,
			convertDatabaseCharsetType("root", "mysql"), null,
		...
		while (rs.next()) {
			...
			ArrayList<Coloums> coloumss = new ArrayList<Coloums>();
			if (rs.getString(4) != null
				&& (rs.getString(4).equalsIgnoreCase("TABLE") || rs
				.getString(4).equalsIgnoreCase("VIEW"))) {
				String tableName = rs.getString(3).toLowerCase();
				...
				while (colRet.next()) {
					...
					coloums.setIsPrimaryKey(isAutoincrement);
					coloumss.add(coloums);
				}
				table.setColoums(coloumss);
				tables.add(table);
			}
		}
		return tables;
